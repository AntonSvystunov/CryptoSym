...
{
    public static class KalynaTransformation
    {
        private static byte[][] sboxes_enc = new byte[4][] {
            new byte[] {
        ...
        private static byte[][] sboxes_dec = new byte[4][] {
            new byte[] {
        ...
        private static byte[][] mds_matrix = new byte[8][]{
        ...
        private static byte[][] mds_inv_matrix = new byte[8][] {
        ...
        public static void SubBytes(ulong[] state, int nb)
        {
            ...
            for (int i = 0; i < nb; ++i)
            {
                state[i] = sboxes_enc[0][s[i] & 0x00000000000000FFUL] |
                                ((ulong)sboxes_enc[1][(s[i] & 0x000000000000FF00UL) >> 8] << 8) |
                                ((ulong)sboxes_enc[2][(s[i] & 0x0000000000FF0000UL) >> 16] << 16) |
                                ((ulong)sboxes_enc[3][(s[i] & 0x00000000FF000000UL) >> 24] << 24) |
                                ((ulong)sboxes_enc[0][(s[i] & 0x000000FF00000000UL) >> 32] << 32) |
                                ((ulong)sboxes_enc[1][(s[i] & 0x0000FF0000000000UL) >> 40] << 40) |
                                ((ulong)sboxes_enc[2][(s[i] & 0x00FF000000000000UL) >> 48] << 48) |
                                ((ulong)sboxes_enc[3][(s[i] & 0xFF00000000000000UL) >> 56] << 56);
            }
        }
        ...
        public static void InvSubBytes(ulong[] state, int nb)
        {
            ...
            for (int i = 0; i < nb; ++i)
            {
                state[i] = sboxes_dec[0][s[i] & 0x00000000000000FFUL] |
                                ((ulong)sboxes_dec[1][(s[i] & 0x000000000000FF00UL) >> 8] << 8) |
                                ((ulong)sboxes_dec[2][(s[i] & 0x0000000000FF0000UL) >> 16] << 16) |
                                ((ulong)sboxes_dec[3][(s[i] & 0x00000000FF000000UL) >> 24] << 24) |
                                ((ulong)sboxes_dec[0][(s[i] & 0x000000FF00000000UL) >> 32] << 32) |
                                ((ulong)sboxes_dec[1][(s[i] & 0x0000FF0000000000UL) >> 40] << 40) |
                                ((ulong)sboxes_dec[2][(s[i] & 0x00FF000000000000UL) >> 48] << 48) |
                                ((ulong)sboxes_dec[3][(s[i] & 0xFF00000000000000UL) >> 56] << 56);
            }
        }
        ...
        public static void MixColumns(ref ulong[] lstate, int nb)
        {
            ...
            for (int col = 0; col < nb; ++col)
            {
                ...
                for (row = sizeof(ulong) - 1; row >= 0; --row)
                {
                    ...
                    {
                        product ^= GMul(state[b + sizeof(ulong) * col], mds_matrix[row][b]);
                    }
        ...
        public static void InvMixColumns(ref ulong[] lstate, int nb)
        {
            ...
            for (int col = 0; col < nb; ++col)
            {
                ...
                for (row = sizeof(ulong) - 1; row >= 0; --row)
                {
                    ...
                    for (b = sizeof(ulong) - 1; b >= 0; --b)
                    {
                        product ^= GMul(state[b + sizeof(ulong) * col], mds_inv_matrix[row][b]);
                    }
                    result |= (ulong)product << (row * sizeof(ulong));
                }
                lstate[col] = result;
